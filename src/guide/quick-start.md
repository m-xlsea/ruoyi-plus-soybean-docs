...
| **pnpm**    | >= 10.5.0  | 必须使用 pnpm，不支持 npm/yarn |
| **Git**     | 最新版本   | 用于代码管理                   |
...
1. **检查生效范围**：确认表格主题配置是否在页面 table 应用效果（目前只在测试菜单有效）
2. **全局替换配置**：如果需要全局生效，请将主题表格配置替换到所有相关 table 组件
3. **刷新页面**：配置修改后刷新页面，确保新样式被加载
4. **清除缓存**：清除浏览器缓存和 localStorage 以避免旧样式覆盖新配置
